...
struct ContentView: View {
    ...
    // calls resnet model and returns model's prediction as output
    func processSnapshot(_ image: UIImage) {
        isLoading = true
        APIManager.shared.predictImage(image: image) { prediction, error in
            DispatchQueue.main.async {
                isLoading = false
                if let prediction = prediction {
                    responseText = "You are at a: \(prediction) intersection"
                } else if let error = error {
                    responseText = "Error predicting intersection type: \(error.localizedDescription)"
                }
                // Update UI elements here as needed
            }
        }
    }
